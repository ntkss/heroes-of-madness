...
export default function MigratePage() {
  ...
  const checkImpact = async () => {
    ...
    try {
      ...
      // 4. Scan Forum Posts
      const postsSnap = await getDocs(collection(database, "posts"));
      const affectedPosts: string[] = [];
      postsSnap.forEach((d) => {
        const p = d.data();
      ...
      addLog(
        `Found ${affectedPosts.length} affected forum posts.`,
        affectedPosts.length > 0 ? "info" : "success",
      );
  ...
  const runMigration = async () => {
    ...
    try {
      ...
      // 4. Migrate Forum Posts
      addLog("Scanning forum posts for references...");
      const postsSnap = await getDocs(collection(database, "posts"));
      let postsUpdated = 0;
      ...
      postsSnap.forEach((postDoc) => {
        ...
        if (changed) {
          batch.update(doc(database, "posts", postDoc.id), {
            mentionedPlayers: newMentionedPlayers || [],
      ...
      addLog(`Total forum posts updated in batch: ${postsUpdated}`);
      ...
      if (typeof window !== "undefined") {
        ...
        localStorage.removeItem("mlbb_generator_posts");
        addLog(
            ...
            <div className="border border-slate-800 bg-slate-950 p-4 mb-6 text-xs">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p>
                    Affected Forum Posts:{" "}
                    <span className="text-cyan-400 font-bold">
                      {previewData.affectedPosts.length}
                    </span>
                  </p>
                </div>
              </div>
