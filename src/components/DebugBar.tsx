...
export default function DebugBar() {
  ...
  useEffect(() => {
    ...
    const updateDimensions = () => {
      ...
      setScreenRes({ width: window.screen.width, height: window.screen.height });
    };
    updateDimensions();
    ...
    const memory = (navigator as any).deviceMemory ? `${(navigator as any).deviceMemory} GB RAM` : "N/A";
        ...
        <div className="w-80 bg-slate-950/95 backdrop-blur-md border-2 border-neon-blue shadow-[0_0_25px_rgba(0,210,255,0.35)] relative overflow-hidden flex flex-col p-4 animate-scaleUp">
          ...
          <div className="flex flex-col gap-2.5 text-[11px] text-[#a0a0c0]">
            ...
            <div className="flex justify-between border-b border-slate-900 pb-1.5">
              <span className="uppercase text-slate-500 font-bold">SYSTEM BUILD</span>
              <span className="text-white font-bold">{buildVersion}</span>
            </div>
            
            {/* Engine Frameworks */}
            <div className="flex justify-between border-b border-slate-900 pb-1.5">
              <span className="uppercase text-slate-500 font-bold">ENGINES</span>
            ...
            {/* DB Status */}
            <div className="flex justify-between border-b border-slate-900 pb-1.5">
              <span className="uppercase text-slate-500 font-bold">DATABASE</span>
            ...
            {/* Network status */}
            <div className="flex justify-between border-b border-slate-900 pb-1.5">
              <span className="uppercase text-slate-500 font-bold">NET CONNECTION</span>
              {isOnline ? (
                <span className="text-green-400 font-bold">ONLINE</span>
              ) : (
                <span className="text-neon-red glow-red font-bold animate-pulse">OFFLINE</span>
              )}
            </div>

            {/* OS & Browser */}
            <div className="flex justify-between border-b border-slate-900 pb-1.5">
              <span className="uppercase text-slate-500 font-bold">CLIENT PLATFORM</span>
            ...
            {/* Viewport */}
            <div className="flex justify-between border-b border-slate-900 pb-1.5">
              <span className="uppercase text-slate-500 font-bold">VIEWPORT SIZE</span>
              <span className="text-white">
            ...
            {/* Resolution */}
            <div className="flex justify-between border-b border-slate-900 pb-1.5">
              <span className="uppercase text-slate-500 font-bold">SCREEN RESOLUTION</span>
              <span className="text-white">
            ...
            {/* System Hardware */}
            <div className="flex justify-between border-b border-slate-900 pb-1.5">
              <span className="uppercase text-slate-500 font-bold">HW SPEC</span>
              <span className="text-white">
            ...
            {/* Local time */}
            <div className="flex justify-between border-b border-slate-900 pb-1.5">
              <span className="uppercase text-slate-500 font-bold">CABINET TIME</span>
              <span className="text-neon-yellow font-pixel text-[9px]">{localTime}</span>
            </div>
            
            {/* Mode */}
            <div className="flex justify-between border-b border-slate-900 pb-1.5">
              <span className="uppercase text-slate-500 font-bold">EXECUTION ENV</span>
              <span className="text-neon-blue capitalize">{process.env.NODE_ENV}</span>
            </div>
          </div>
        ...
        <span className="font-pixel text-[9px] tracking-wider flex items-center gap-1.5">
          <span
            className={`inline-block w-1.5 h-1.5 rounded-full ${
              isFirebaseConfigured ? "bg-neon-yellow animate-pulse" : "bg-neon-red"
            }`}
          />
